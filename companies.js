...
export const companiesPage = () => {
  ...
  return `<main class="page">
    ...
    <div class="section-head"><div class="search-box">${icon('search')}<input id="company-search" value="${initialQuery.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}" placeholder="Search company, contact or owner…"></div><div class="section-meta">${rows.length} companies · Permanent registry${isPersonalOutreachView()?' · Your pipeline':''}</div></div>
