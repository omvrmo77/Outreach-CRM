...
import { getCompany, getActivities, getAccount, getAccounts, getChangeHistory, getCompanyContacts } from './crmState.js?v=20260917-multicontact1';
...
export const companyProfilePage = (name,{invalidRoute=false}={}) => {
  ...
  return `<main class="page">
    ...
    <section class="relationship-summary grid grid-5">
      ...
      <div class="summary-stat card"><span>First activity</span><strong>${first?formatDeviceDateTime(first.at):'Date unavailable'}</strong><small>${first?`Local · GMT-04 ${formatDateTime(first.at)}`:''}</small></div>
      <div class="summary-stat card"><span>Last activity</span><strong>${last?formatDeviceDateTime(last.at):'Date unavailable'}</strong><small>${last?`Local · GMT-04 ${formatDateTime(last.at)}`:''}</small></div>
    ...
    ${canMutate?`<div class="action-modal" id="activity-action-modal" aria-hidden="true">
      ...
      <div class="action-modal-card card activity-detail-modal">
        ...
        <form id="activity-action-form">
          ...
          <div class="field workday-field"><label>Count toward workday</label><input id="activity-workday" type="date" value="${getWorkspaceDateKey(now)}"><small>This controls reporting only. It can be the GMT-04 activity date or the immediately previous day.</small></div>
